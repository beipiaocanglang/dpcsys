...
//			System.out.println("ip:host--->" + getRemoteHost(request));
//			Map<String, String> map = request.getParameterMap();
//			if(map != null){
//				for (String key : map.keySet()) {
//				   System.out.println("key= "+ key + " and value= " + map.get(key) != null ? map.get(key).toString() : "");
//				}
//			}
            ...
            logger.error("ex ： {}", ex);
            JsonCoreUtil.outPutJson(response, JsonCoreUtil.toJSONString(maps));
            return null;
        } catch (Exception e) {
            maps.put("code", new Date().getTime() + "");
            maps.put("begin_date", DateCoreUtil.formatCnDateYMDHMSSS(new Date()));
